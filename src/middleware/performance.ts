...
export const requestTimer = (req: Request, res: Response, next: NextFunction): void => {
  ...
  res.end = function(chunk: any, encoding: BufferEncoding): Response {
    ...
    if (res.statusCode >= 500) {
      logger.error(`服務器錯誤: ${JSON.stringify(logData)}`);
    } else if (res.statusCode >= 400) {
      logger.warn(`客戶端錯誤: ${JSON.stringify(logData)}`);
    } else {
      logger.info(`請求完成: ${logData.method} ${logData.url} ${logData.status} ${logData.responseTime}`);
    }
...
// 內存使用監控
export const memoryUsage = (req: Request, res: Response, next: NextFunction): void => {
  ...
  res.on('finish', () => {
    ...
    if (memUsed.heapUsed > 5) { // 超過 5MB 記憶體增加
      logger.warn(`記憶體用量高: ${req.method} ${req.originalUrl}, 增加: ${memUsed.heapUsed.toFixed(2)}MB`);
    }
  });
...
// 定期記錄系統狀態
export const startSystemMonitoring = (interval = 5 * 60 * 1000): NodeJS.Timeout => { // 默認 5 分鐘
  return setInterval(() => {
    const mem = process.memoryUsage();
    const cpuUsage = process.cpuUsage();
    
    logger.info(`系統狀態: 記憶體 RSS: ${(mem.rss / 1024 / 1024).toFixed(2)}MB, ` +
